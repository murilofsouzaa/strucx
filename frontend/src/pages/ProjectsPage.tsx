...
type CategoryFilter = 'Todos' | 'Comercial' | 'Infraestrutura' | 'Industrial' | 'Residencial' | 'Retrofit';
  ...
  return (
    <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-2 bg-slate-50 border border-slate-200 rounded-sm mb-16">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Buscar por obra, cidade ou cliente..."
            className="w-full bg-white border border-slate-300 rounded-sm pl-9 pr-3 py-2 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-[#0284C7]"
          />
        </div>
      </div>

      {/* Heatherwick-Inspired Editorial Projects Grid */}
      ...
      {filteredProjects.length === 0 && (
        <div className="text-center py-20 bg-slate-50 rounded-sm border border-slate-200 mb-16">
          <Buildings size={48} className="mx-auto text-slate-400 mb-3" />
          <h3 className="text-base font-heading font-bold text-[#0F172A] mb-1">Nenhum projeto encontrado</h3>
          <p className="text-xs font-body text-slate-500">Tente buscar por outro termo ou selecione a categoria "Todos".</p>
        </div>
      )}
